...
//	@PersistenceContext
	...
	public void adiciona(Movimentacao movimentacao) {
		...
		if (movimentacao.getValor().compareTo(BigDecimal.ZERO) < 0) {
		    throw new ValorInvalidoException("Movimentacao negaiva");
//			throw new RuntimeException("Movimentacao negativa");
	...
	public List<ValorPorMesEAno> listaMesesComMovimentacoes(Conta conta, TipoMovimentacao tipo) {
		...
		String jpql = "select new br.com.caelum.financas.modelo.ValorPorMesEAno(month(m.data)," +
		              "year(m.data), sum(m.valor)) from Movimentacao m where m.conta = :conta " +
				      "and m.tipoMovimentacao = :tipo group by year(m.data), month(m.data)" +
